...
public class MultiDimensionPolynomial
    ...
    private static final TreeMap<Integer, Long> FACTORIAL_CACHE = new TreeMap<>(Map.ofEntries(
            Map.entry(0, 1L), Map.entry(1, 1L)
    ));
    ...
    public double[][] unflat(double[] flatWeights, int nOfOutputs) {
        if (flatWeights.length % nOfOutputs != 0 || flatWeights.length / nOfOutputs != computeNOfCombinations(nOfInputs, degree)) {
            throw new IllegalArgumentException("Bruh");
        }
